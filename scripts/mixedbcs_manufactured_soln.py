...
output_path = Path("convergence_static")
...
values = [0] + list(np.logspace(-18, 0, 5, base=2))
                ...
                for U_scale in [0, 1]:
                    for P_scale in [0, 1]:
                        ...
                        if plot_result:
                            ...
                            fig.savefig(fig_path / f"bcs_{setup}--u_{U_scale(0)}-p_{P_scale(0)}--nT_{nT:02d}-nX_{nX:02d}--ti_00.png", dpi=200)
                            plt.close(fig)
                        ...
                        for i in trange(nT):
                            ...
                            errors['u (Linf)'].append(np.max(np.abs(est_u.vector() - bc_u.vector())))
                            ...
                            errors['pT (Linf)'].append(np.max(np.abs(est_p_total.vector() - bc_p_total.vector())))
                            errors['pF (H1)'].append(pde.errornorm(true_p, solution.split()[2], 'h1'))
                            errors['pF (L2)'].append(pde.errornorm(true_p, solution.split()[2], 'l2'))
                            errors['pF (Linf)'].append(np.max(np.abs(est_p.vector() - bc_p.vector())))
                            pd.DataFrame(errors).to_csv(output_path / "errors.csv")

                        i = 0
                        if plot_result:
                            fig, axes = plot_solution(true_u, true_p, solution.split()[0], solution.split()[2], bc_V, bc_Q)
                            fig.savefig(fig_path / f"bcs_{setup}--u_{U_scale(0)}-p_{P_scale(0)}--nT_{nT:02d}-nX_{nX:02d}--ti_{i+1:02d}.png", dpi=200)
                            plt.close(fig)
