...
class BiotSystem:
    ...
    def _get_function_space(self):
        P2 = pde.VectorElement("CG", self.mesh.ufl_cell(), 2)
        P1 = pde.FiniteElement("CG", self.mesh.ufl_cell(), 1)
        P2P1 = pde.MixedElement([P2, P1])
        W = pde.FunctionSpace(self.mesh, P2P1)
        return W
    ...
    def get_lhs(self):
        ...
        u, p = self.trial_functions
        v, q = self.test_functions

        a = (
              inner(2*mu*epsilon(u), epsilon(v)) + lambda_*nabla_div(u)*nabla_div(v) - alpha * p * q
                                                 + alpha*div(u)*q + (alpha*alpha / lambda_)*p*q - dt*kappa*inner(nabla_grad(p), nabla_grad(q))
        )*dx
        return a
    ...
    def get_rhs(self):
        ...
        dt = self.dt
        v, q = self.test_functions
        u_, p_ = pde.split(self.previous_solution)

        L = inner(f, v)*dx + dt*g*q*dx
        timestep = + alpha*div(u_)*q + (alpha*alpha / lambda_)*p_*q * dx
        return L + timestep
